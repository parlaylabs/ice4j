...
/**
 * Uses the Amazon AWS APIs to retrieve the public IPv6 and private IPv4 addresses
 * for an EC2 instance.
 * 
 */
public class AwsIPv6CandidateHarvester
    extends MappingCandidateHarvester
{
    /**
     * The <tt>Logger</tt> used by the <tt>AwsIPv6CandidateHarvester</tt>
     * class and its instances for logging output.
     */
    private static final Logger logger
        = Logger.getLogger(AwsIPv6CandidateHarvester.class.getName());
    ...
    private static final String PUBLIC_IPV6_URL
    = "http://169.254.169.254/latest/meta-data/network/interfaces/macs/${MAC}/ipv6s";
    
    private static final String PLACEHOLDER_MAC_VALUE
    = "${MAC}";
     ...
     */
    public AwsIPv6CandidateHarvester()
    {
        super();
    }
     ...
     * <tt>http://169.254.169.254/latest/meta-data/network/interfaces/macs/${MAC}/ipv6s</tt> to learn the
    ...
    private static synchronized void obtainEC2Addresses()
    {
        ...
        String macStr = null;
        String publicIPv6Str = null;

        try
        {
            localIPStr = fetch(LOCAL_IP_URL);
            macStr = fetch(MAC_URL);
            publicIPv6Str = fetch(PUBLIC_IPV6_URL.replace(PLACEHOLDER_MAC_VALUE, macStr));

            //now let's cross our fingers and hope that what we got above are
            //real IP addresses
            face = new TransportAddress(localIPStr, 9, Transport.UDP);
            mask = new TransportAddress(publicIPv6Str, 9, Transport.UDP);

            logger.info("Detected AWS local IP: " + face);
            logger.info("Detected AWS public IPv6: " + mask);
        }
        catch (Exception exc)
        {
            ...
            logger.info("String for public IPv6: " + publicIPv6Str);
        }
    }
